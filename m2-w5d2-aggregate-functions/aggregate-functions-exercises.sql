...
SELECT SUM(surfacearea * .01) AS sum_surfacearea, continent
FROM country
GROUP BY continent
...
SELECT ABS(gnpold - gnp) AS gnp_absolut, (gnp - gnpold) AS gnp_difference, name
FROM country
WHERE gnp>0 AND gnpold>0
ORDER BY gnp_absolut;
...
SELECT COUNT(district), countrycode
FROM city
GROUP BY countrycode
ORDER BY count DESC;
